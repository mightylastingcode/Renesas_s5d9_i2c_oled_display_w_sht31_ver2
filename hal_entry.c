...
void hal_entry(void)
{
    ...
    // set internal i2c bus for EN210
     ...
     writedisplaybuffer(0, 40, msg, strlen(msg));
     sprintf(msg,"PROJECT");
     writedisplaybuffer(0, 48, msg, strlen(msg));
     sprintf(msg,"11/9/2017");
     writedisplaybuffer(0, 56, msg, strlen(msg));
     ...
     while (1) {
         ...
         // set i2c for SHT31 operation
          err = g_ext_i2c_bus_slave_addr_set(SHT31_I2C_ADDRESS);
          APP_ERROR_TRAP(err)

         // Read SHT31 Status
         //err = readsht31status (&sht31status);
         //APP_ERROR_TRAP(err)

         // Read SHT31 Temperature and Humidity values
         err = readsht31_temperature_humidity(&g_temperatureC, &g_temperatureF, &g_humidityP);
         APP_ERROR_TRAP(err)
